...
def processar_data(texto):
    # Corrigir datas com 1 dígito no dia ou no mês
    padroes_incompletos = re.findall(r"\b\d{1,2}[-/:]\d{1,2}\b", texto)
    texto_corrigido = texto

    for data in padroes_incompletos:
        ...
        if len(partes) == 2:
            ...
            if len(mes) == 1:
                mes = '0' + mes
            data_corrigida = f"{dia}/{mes}"
            texto_corrigido = texto_corrigido.replace(data, data_corrigida)

    # Agora sim, pegar só datas válidas no formato dd/mm
    datas_validas = re.findall(r"\b\d{2}[-/:]\d{2}\b", texto_corrigido)

    if not datas_validas:
        return "Nenhuma data válida encontrada"
    
    ultima_data = datas_validas[-1]
    parte_texto = texto_corrigido.split(ultima_data, 1)
...
def formatar_solicitacao(texto, condicoes):
    if texto:
        ...
        padrao = rf'(A1|ANEXAR|SOLICITAR|ANX|PEDIR)\s+(.*?)(?:\s+(?:{palavras})\s*|\s*\.)'
...
def formatar_texto(nome, procedimento, solicitacao, questionamento, consulta_plano, endereco):
    resultado = []
    if nome:
        resultado.append(f'Olá, aqui é do *HAPVIDA NOTREDAME* falo com *{nome}*?\n\n') 
...
def formatar_texto_parecer(nome, codigo, procedimentos, info_medico):
    ...
    resultado = f"PARECER / HAPVIDA NOTREDAME / {header}\nBom dia,\n\n{info}\n\n{header}{codigo_nome_procedimento}"
...
def texto_nome(nome):
    return f'Olá, aqui é do *HAPVIDA NOTREDAME* falo com *{nome}*?\n\n'
